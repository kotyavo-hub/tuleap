...
class TransferIssuesCommand extends GenericTransferCommand
{
    ...
    protected function transfer(InputInterface $input, SymfonyStyle $output): int
    {
        ...
        foreach ($redmineIssues as $issueId => $redmineIssue) {
            ...
            $issueAuthor = new PFUser(['user_id' => $redmineIssue[RedmineIssueColumnEnum::AUTHOR_ID]]);
            ...
            $this->addProjectMembers($tracker, $tuleapArtifact);
    ...
    private function convertValue(
        ...
        switch ($issueField) {
            ...
            case RedmineIssueColumnEnum::PROJECT_ID:
                return $this->refRepo->getTuleapId(EntityTypeEnum::PROJECT(), $value);
            ...
            case RedmineIssueColumnEnum::AUTHOR_ID:
                return $this->refRepo->getTuleapId(EntityTypeEnum::USER(), $value);
            ...
            // see transferParentIds
            case RedmineIssueColumnEnum::PARENT_ID: break;
        }
    ...
    private function transferParentIds(SymfonyStyle $output, int $trackerId, array $redmineIssues, array $issueToArtifact): void
    {
        ...
        foreach ($redmineIssues as $issueId => $redmineIssue) {
            ...
            $tuleapParentId = $this->refRepo->getTuleapId(EntityTypeEnum::ISSUE(), $redmineIssue[RedmineIssueColumnEnum::PARENT_ID]);

            $artifact = $issueToArtifact[$issueId];

            $changeset = $artifact->createNewChangeset(
                [
                    $fieldsByLabel[RedmineIssueColumnEnum::PARENT_ID][TuleapTrackerFieldColumnEnum::ID] => ['new_values' => $tuleapParentId],
                ],
                '',
    ...
    private function allowUserBindedFieldsHaveAnyUser(): void
    {
        ...
        $backupableFields = $tuleapDb->run(
            '
                SELECT *
                FROM ' . TuleapTableEnum::TRACKER_FIELD_LIST_BIND_USERS. '
                WHERE ' . FieldListBindUserColumnEnum::VALUE_FUNCTION .  ' like "*group_members*"
            '
        );
        ...
        foreach ($backupableFields as $backupableField) {
            ...
            $tuleapDb->update(
                ...
                [FieldListBindUserColumnEnum::VALUE_FUNCTION => ''],
     ...
     */
    private function addProjectMembers(Tracker $tracker, array $artifact): void
    {
